...
function isBenignRuntimeCancellation(value) {
  ...
  if (name === "aborterror" || name === "cancelederror" || name === "cancelerror") return true;
  if (code === "err_canceled" || code === "abort_err") return true;
  if (value.__CANCEL__ === true) return true;
  if (message === "canceled" || message === "cancelled") return true;
  if (message === "the operation was aborted" || message === "operation aborted") return true;
  return false;
}

function isBenignRuntimeCancellationEvent(event) {
  return isBenignRuntimeCancellation(event?.reason) || isBenignRuntimeCancellation(event?.error);
}
...
globalThis.addEventListener?.("error", (event) => {
  if (isBenignRuntimeCancellationEvent(event)) return;
