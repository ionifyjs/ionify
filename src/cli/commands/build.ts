...
export function rerouteDepsArtifacts(options: {
  ...
  // Authority is no longer transported as an out-of-band set. Each module
  ...
  for (const chunk of plan.chunks) {
    ...
    for (const mod of chunk.modules) {
      ...
      if (!artifact && !isNodeModules) {
        // Workspace Transform-domain source is Transform-owned. Stamp the
      ...
      if (artifact) {
        ...
        // DPL owns this artifact's identity + integrity contract. Stamp the
    ...
    for (const target of declared) {
      ...
      if (!owners.has(targetId)) {
        ...
        const targetModule: BuildPlan["chunks"][number]["modules"][number] = {
          ...
          // DPL-injected dependency/shared/graph target uses the DPL contract.
 ...
 * proof writer, ACKs, and RELEASES the generation bytes (codeA/mapA/codeB) — bounded
...
function runCanonicalColdDerivation(opts: {
  ...
  try {
    for (;;) {
      ...
      for (const g of wave) {
        try {
          // Material projection: base A + define B + proof marker-last.
          materializeCanonicalGeneration(
...
export async function runBuildCommand(options: BuildOptions = {}) {
  try {
    ...
    // The expected Transform-artifact contract for a consumed JS module,
    ...
    for (const [id, meta] of moduleMetaById.entries()) {
      ...
      if (meta.kind === "css") {
        ...
        // Authority dispatch is based solely on the plan-carried `proofKind`.
        ...
        if (proofKind === "DplContentHash") {
          ...
          // Invariant: a consumable JS module reaching admission with no
          ...
          throw new Error(
            `[Ionify] Non-self-describing plan: JS consumable '${id}' ` +
      ...
      // without re-running the transform worker. Admit the BASE proof first
    ...
    for (const job of defineJobs) {
      ...
      try {
        // Write the derived variant + its Transform proof atomically, and
    ...
    if (jobs.length > 0) {
      ...
      for (const job of jobs) {
        ...
        if (isJs) {
          // Write base + define-variant artifacts each with their own
